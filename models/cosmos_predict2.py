...
class CosmosPredict2Pipeline(BasePipeline):
    ...
    def prepare_inputs(self, inputs, timestep_quantile=None):
        ...
        active_control_ref_ids = inputs.get('active_control_ref_ids', None)
        ...
        if active_control_ref_ids is None and active_control_refs is not None:
            active_control_ref_ids = torch.tensor(
                [CONTROL_REF_IDS[ref] for ref in active_control_refs],
                dtype=torch.long,
                device=noisy_latents.device,
            )
        elif active_control_ref_ids is None:
            active_control_ref_ids = torch.empty(0, dtype=torch.long, device=noisy_latents.device)
        else:
            active_control_ref_ids = active_control_ref_ids.to(device=noisy_latents.device, dtype=torch.long)
        if active_control_ref_ids.ndim == 1:
            active_control_ref_ids = active_control_ref_ids.unsqueeze(0).expand(bs, -1)
